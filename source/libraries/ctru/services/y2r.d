...
enum Y2RUInputFormat : ubyte
{
    ...
    yuv422_indiv_16 = 0x2, ///< 16-bit per component, planar YUV 4:2:2, 32bpp, (1 Cr & Cb sample per 2x1 Y samples).\n Usually named YUV422P16.
    yuv420_indiv_16 = 0x3, ///< 16-bit per component, planar YUV 4:2:0, 24bpp, (1 Cr & Cb sample per 2x2 Y samples).\n Usually named YUV420P16.
    yuv422_batch    = 0x4  ///< 8-bit per component, packed YUV 4:2:2, 16bpp, (Y0 Cb Y1 Cr).\n Usually named YUYV422.
}
...
enum Y2RUOutputFormat : ubyte
{
    ...
    rgb_16_555 = 0x2, ///< 16-bit RGBA5551. The alpha bit is the 7th bit of the alpha value set by @ref Y2RU_SetAlpha
    rgb_16_565 = 0x3  ///< 16-bit RGB565.
}
...
enum Y2RURotation : ubyte
{
    ...
    clockwise_180 = 0x2, ///< Clockwise 180 degrees.
    clockwise_270 = 0x3  ///< Clockwise 270 degrees.
}
...
{
    ushort rgb_Y; ///< RGB per unit Y.
    ushort r_V; ///< Red per unit V.
    ushort g_V; ///< Green per unit V.
    ushort g_U; ///< Green per unit U.
    ushort b_U; ///< Blue per unit U.
    ushort r_offset; ///< Red offset.
    ushort g_offset; ///< Green offset.
    ushort b_offset; ///< Blue offset.
}
...
enum Y2RUStandardCoefficient : ubyte
{
    ...
    itu_r_bt_601_scaling = 0x2, ///< Coefficients from the ITU-R BT.601 standard with TV ranges.
    itu_r_bt_709_scaling = 0x3  ///< Coefficients from the ITU-R BT.709 standard with TV ranges.
}
...
struct Y2RU_ConversionParams
{
    ...
    ///< Value passed to @ref Y2RU_SetRotation
...
{
    ushort w0_xEven_yEven; ///< Weight 0 for even X, even Y.
    ushort w0_xOdd_yEven; ///< Weight 0 for odd X, even Y.
    ushort w0_xEven_yOdd; ///< Weight 0 for even X, odd Y.
    ushort w0_xOdd_yOdd; ///< Weight 0 for odd X, odd Y.
    ushort w1_xEven_yEven; ///< Weight 1 for even X, even Y.
    ushort w1_xOdd_yEven; ///< Weight 1 for odd X, even Y.
    ushort w1_xEven_yOdd; ///< Weight 1 for even X, odd Y.
    ushort w1_xOdd_yOdd; ///< Weight 1 for odd X, odd Y.
    ushort w2_xEven_yEven; ///< Weight 2 for even X, even Y.
    ushort w2_xOdd_yEven; ///< Weight 2 for odd X, even Y.
    ushort w2_xEven_yOdd; ///< Weight 2 for even X, odd Y.
    ushort w2_xOdd_yOdd; ///< Weight 2 for odd X, odd Y.
    ushort w3_xEven_yEven; ///< Weight 3 for even X, even Y.
    ushort w3_xOdd_yEven; ///< Weight 3 for odd X, even Y.
    ushort w3_xEven_yOdd; ///< Weight 3 for even X, odd Y.
    ushort w3_xOdd_yOdd; ///< Weight 3 for odd X, odd Y.
}
 ...
 * @note Prefer using @ref Y2RU_SetConversionParams if you have to set multiple parameters.
 */
Result Y2RU_SetInputLineWidth(ushort line_width);

/**
 * @brief Gets the configured input line width.
 * @param line_width Pointer to output the line width to.
 */
Result Y2RU_GetInputLineWidth(ushort* line_width);
 ...
 */
Result Y2RU_SetSendingY(const(void)* src_buf, uint image_size, short transfer_unit, short transfer_gap);
 ...
 * The actual transfer will only happen after calling @ref Y2RU_StartConversion.
 */
Result Y2RU_SetSendingU(const(void)* src_buf, uint image_size, short transfer_unit, short transfer_gap);
 ...
 * The actual transfer will only happen after calling @ref Y2RU_StartConversion.
 */
Result Y2RU_SetSendingV(const(void)* src_buf, uint image_size, short transfer_unit, short transfer_gap);
 ...
 * The actual transfer will only happen after calling @ref Y2RU_StartConversion.
 */
Result Y2RU_SetSendingYUYV(const(void)* src_buf, uint image_size, short transfer_unit, short transfer_gap);
 ...
 */
Result Y2RU_SetReceiving(void* dst_buf, uint image_size, short transfer_unit, short transfer_gap);
 ...
 */
Result Y2RU_PingProcess(ubyte* ping);

/// Initializes the Y2R driver.
Result Y2RU_DriverInitialize();

/// Terminates the Y2R driver.
Result Y2RU_DriverFinalize();
